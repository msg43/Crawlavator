"""
Dwarkesh Patel Podcast Plugin
Transcripts available on dwarkesh.com (Substack)
"""
...
@register_site
class DwarkeshSite(BaseSite):
    """Dwarkesh Patel podcast site plugin"""
    
    SITE_ID = "dwarkesh"
    SITE_NAME = "Dwarkesh Patel"
    REQUIRES_AUTH = False
    ...
    def index_content(self, progress_callback=None) -> List[ContentItem]:
        ...
        if progress_callback:
            progress_callback("Fetching Dwarkesh Patel RSS feed...")
        ...
        try:
            ...
            for entry in feed.entries:
                try:
                    ...
                    # Create unique ID
                    item_id = f"dwarkesh_{self._sanitize_id(title)}"
    ...
    def download_item(self, item: ContentItem, output_dir: str,
        ...
        """Download episode transcript from Substack page"""
        ...
        try:
            ...
            # Substack posts are typically in .body or .post-content class
            transcript_content = None
            
            # Strategy 1: Look for Substack-specific classes
            transcript_div = soup.find(['div'], class_=re.compile(r'body|post-content|available-content'))
            if transcript_div:
                transcript_content = transcript_div
            
            # Strategy 2: Look for article or main content
            if not transcript_content:
                transcript_content = soup.find(['article', 'main'])
            
            # Strategy 3: Look for content divs
            if not transcript_content:
                transcript_content = soup.find('div', class_=re.compile(r'content'))
            ...
            header += f"Source: Dwarkesh Patel Podcast\n"
    ...
    def _extract_metadata(self, soup: BeautifulSoup, item: ContentItem) -> Dict[str, Any]:
        ...
        # Typical format: "Guest Name - Topic" or "Episode with Guest Name"
        guest_match = re.search(r'^([^-]+?)\s*[-–—]', item.title)
        if guest_match:
            metadata['guest'] = guest_match.group(1).strip()
        else:
            # Alternative: "Episode with X" or "X on Y"
            guest_match = re.search(r'(?:with|Episode:)\s+([^-–—]+)', item.title, re.I)
            if guest_match:
                metadata['guest'] = guest_match.group(1).strip()
        
        # Try to find episode number
        episode_match = re.search(r'#?(\d+)[:\s-]', item.title)
        if episode_match:
            metadata['episode_number'] = episode_match.group(1)
    ...
    def _extract_formatted_text(self, content_div) -> str:
        ...
        for element in content_div.find_all(['script', 'style', 'nav', 'footer', 'header']):
            element.decompose()
