...
@register_site
class ExcessReturnsSite(BaseSite):
    """Excess Returns podcast site plugin"""
    
    SITE_ID = "excess_returns"
    SITE_NAME = "Excess Returns"
    REQUIRES_AUTH = False
    ASSET_TYPES = ["transcript", "audio"]
    CATEGORIES = ["podcast"]
    IMPORT_SOURCE = "excessreturnspod.com"
    
    RSS_URL = "https://feeds.captivate.fm/excess-returns/"
    ...
    def index_content(self, progress_callback=None) -> List[ContentItem]:
        ...
        if progress_callback:
            progress_callback("Fetching Excess Returns RSS feed...")
    ...
    def download_item(self, item: ContentItem, output_dir: str,
        ...
        try:
            ...
            # Look for transcript
            transcript = soup.find(['div', 'article'], class_=re.compile(r'transcript|content|episode'))
            ...
            if transcript and len(transcript.get_text(strip=True)) > 200:
                ...
                header += f"Source: Excess Returns\n\n---\n\n"
