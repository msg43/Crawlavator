"""
Hidden Forces Podcast Plugin
Demetri Kofinas podcast with some transcripts on hiddenforces.io
"""
...
@register_site
class HiddenForcesSite(BaseSite):
    """Hidden Forces podcast site plugin"""
    
    SITE_ID = "hidden_forces"
    SITE_NAME = "Hidden Forces"
    REQUIRES_AUTH = False
    ASSET_TYPES = ["transcript", "audio"]
    CATEGORIES = ["podcast"]
    IMPORT_SOURCE = "hiddenforces.io"
    
    RSS_URL = "https://feeds.megaphone.fm/hiddenforces"
    ...
    def index_content(self, progress_callback=None) -> List[ContentItem]:
        ...
        if progress_callback:
            progress_callback("Fetching Hidden Forces RSS feed...")
    ...
    def download_item(self, item: ContentItem, output_dir: str,
        ...
        try:
            # Try to get transcript from hiddenforces.io
            ...
            # Look for transcript
            transcript = soup.find(['div', 'article'], class_=re.compile(r'transcript|content|entry'))
