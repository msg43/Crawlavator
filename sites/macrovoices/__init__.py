"""
MacroVoices Podcast Plugin
Finance-focused podcast with transcripts on macrovoices.com
"""
...
@register_site
class MacroVoicesSite(BaseSite):
    """MacroVoices podcast site plugin"""
    
    SITE_ID = "macrovoices"
    SITE_NAME = "MacroVoices"
    REQUIRES_AUTH = False
    ...
    def index_content(self, progress_callback=None) -> List[ContentItem]:
        ...
        if progress_callback:
            progress_callback("Fetching MacroVoices RSS feed...")
        ...
        try:
            ...
            for entry in feed.entries:
                try:
                    ...
                    episode_num = None
                    num_match = re.search(r'#?(\d{3,4})', title)
                    if num_match:
                        episode_num = num_match.group(1)
                    
                    item_id = f"mv_{episode_num}" if episode_num else f"mv_{self._slugify(title)}"
    ...
    def download_item(self, item: ContentItem, output_dir: str,
        ...
        try:
            ...
            # Look for transcript
            transcript = soup.find(['div', 'article'], class_=re.compile(r'transcript|content'))
