"""
Odd Lots Podcast Plugin
Bloomberg podcast with transcripts on bloomberg.com
"""
...
@register_site
class OddLotsSite(BaseSite):
    """Odd Lots podcast site plugin"""
    ...
    def index_content(self, progress_callback=None) -> List[ContentItem]:
        ...
        if progress_callback:
            progress_callback("Fetching Odd Lots RSS feed...")
    ...
    def download_item(self, item: ContentItem, output_dir: str,
        ...
        try:
            # Try to get transcript from Bloomberg
            response = self.session.get(item.url, timeout=30)
            response.raise_for_status()
            
            soup = BeautifulSoup(response.content, 'lxml')
            
            # Look for transcript on Bloomberg page
            transcript = soup.find(['div', 'article'], class_=re.compile(r'transcript|body-copy|article-body'))
            ...
            if transcript and len(transcript.get_text(strip=True)) > 200:
                ...
                header += f"Source: Odd Lots (Bloomberg)\n\n---\n\n"
